...
const SECONDARY_EMAIL = process.env.SECONDARY_EMAIL ?? "";
...
function getAdminRecipients(): string {
  ...
  if (recipients.length === 0) {
    throw new Error("No admin email recipients configured. Set ADMIN_EMAIL, PRIMARY_EMAIL, or SECONDARY_EMAIL.");
  }
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; border: 1px solid #e5e7eb; border-radius: 8px;">
        ...
        <p style="color: #4b5563; line-height: 1.6;">
          ...
          <a href="https://wa.me/${PRIMARY_PHONE.replace(/\D/g, "")}" style="color: #FF6028;">WhatsApp: ${PRIMARY_PHONE}</a>
        </p>
        ...
        <p style="font-size: 12px; color: #9ca3af; text-align: center;">
          ${SITE_NAME} &nbsp;|&nbsp; Erode, Tamil Nadu, India<br />
...
export async function sendServiceUpdateAlert(serviceName: string, updateType: "created" | "updated" | "deleted"): Promise<void> {
  try {
    const transporter = createTransporter();
    const recipients = getAdminRecipients();
    await transporter.sendMail({
      from: `"${SITE_NAME}" <${SMTP_EMAIL}>`,
      to: recipients,
      subject: `Service ${updateType.charAt(0).toUpperCase() + updateType.slice(1)}: ${serviceName} - ${SITE_NAME}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; border: 1px solid #e5e7eb; border-radius: 8px;">
          <h2 style="color: #FF6028;">Service ${updateType.charAt(0).toUpperCase() + updateType.slice(1)}</h2>
          <p style="color: #4b5563;">The service <strong>${serviceName}</strong> has been <strong>${updateType}</strong> on ${SITE_NAME}.</p>
          <p style="font-size: 12px; color: #9ca3af;">This is an automated alert from <a href="${SITE_URL}" style="color: #FF6028;">${SITE_URL}</a></p>
        </div>
      `,
    });
  } catch (err) {
    console.error("Failed to send service update alert:", err);
  }
}

export async function sendProductUpdateAlert(productName: string, updateType: "created" | "updated" | "deleted"): Promise<void> {
  try {
    const transporter = createTransporter();
    const recipients = getAdminRecipients();
    await transporter.sendMail({
      from: `"${SITE_NAME}" <${SMTP_EMAIL}>`,
      to: recipients,
      subject: `Product ${updateType.charAt(0).toUpperCase() + updateType.slice(1)}: ${productName} - ${SITE_NAME}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; border: 1px solid #e5e7eb; border-radius: 8px;">
          <h2 style="color: #FF6028;">Product ${updateType.charAt(0).toUpperCase() + updateType.slice(1)}</h2>
          <p style="color: #4b5563;">The product <strong>${productName}</strong> has been <strong>${updateType}</strong> on ${SITE_NAME}.</p>
          <p style="font-size: 12px; color: #9ca3af;">This is an automated alert from <a href="${SITE_URL}" style="color: #FF6028;">${SITE_URL}</a></p>
        </div>
      `,
    });
  } catch (err) {
    console.error("Failed to send product update alert:", err);
  }
}

export async function sendAdminLoginAlert(adminEmail: string, ipAddress?: string): Promise<void> {
  try {
    const transporter = createTransporter();
    const recipients = getAdminRecipients();
    await transporter.sendMail({
      from: `"${SITE_NAME}" <${SMTP_EMAIL}>`,
      to: recipients,
      subject: `Admin Login Alert - ${SITE_NAME}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; border: 1px solid #e5e7eb; border-radius: 8px;">
          <h2 style="color: #FF6028;">Admin Login Detected</h2>
          <table style="width: 100%; border-collapse: collapse;">
            <tr><td style="padding: 8px 0; color: #6b7280; width: 120px;"><strong>Email</strong></td><td style="padding: 8px 0;">${adminEmail}</td></tr>
            <tr><td style="padding: 8px 0; color: #6b7280;"><strong>Time</strong></td><td style="padding: 8px 0;">${new Date().toLocaleString()}</td></tr>
            ${ipAddress ? `<tr><td style="padding: 8px 0; color: #6b7280;"><strong>IP</strong></td><td style="padding: 8px 0;">${ipAddress}</td></tr>` : ""}
          </table>
          <p style="margin-top: 16px; font-size: 12px; color: #9ca3af;">If this was not you, please change your password immediately.</p>
        </div>
      `,
    });
  } catch (err) {
